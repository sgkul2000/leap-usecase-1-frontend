...
const ColumnsTable = (props) => {
  ...
  const openUpdateModal = async (values) => {
    ...
    setPriceInput(values.price)
    setStatusInput(values.status)
    onUpdateOpen()
  }
  ...
  const deleteItem = ( ) => {
    axios
    .delete(process.env.REACT_APP_BASE_URL + "/item/" + currentRow)
    .then( () => {
      props.fetchData()
      setCurrentRow("")
      onDeleteClose()
  ...
  const createItem = () => {
    axios
    .post(`${process.env.REACT_APP_BASE_URL}/item/${storeInput}/${categoryInput}/`, {
      name: nameInput,
      price: priceInput,
      status: statusInput
    })
  ...
  const updateItem = () => {
    axios
    .put(process.env.REACT_APP_BASE_URL + "/item/" + currentRowValues.id + "/", {
      name: nameInput,
      price: priceInput,
      status: statusInput
    })
    ...
    <Card extra={"w-full pb-10 p-4 h-full"}>
        ...
        <ModalContent className="!z-[1002] !m-auto !w-max min-w-[350px] !max-w-[85%] md:top-[12vh]">
          <ModalBody>
            <Card extra="px-[30px] pt-[35px] pb-[40px] max-w-[450px] flex flex-col !z-[1004]">
              <h1 className="mb-[20px] text-2xl font-bold">Create Item</h1>
              ...
              <div className="flex gap-2">
                <button onClick={() => { createItem()}} className="rounded-xl bg-brand-500 px-5 py-3 text-base font-medium text-white transition duration-200 hover:bg-brand-600 active:bg-brand-700 dark:bg-brand-400 dark:text-white dark:hover:bg-brand-300 dark:active:bg-brand-200">
                  Submit
                </button>
                <button
      ...
      <Modal isOpen={isDeleteOpen} onClose={closeDeleteModal} className="!z-[1010]">
        ...
        <ModalContent className="!z-[1002] !m-auto !w-max min-w-[350px] !max-w-[85%] md:top-[12vh]">
          <ModalBody>
            <Card extra="px-[30px] pt-[35px] pb-[40px] max-w-[450px] flex flex-col !z-[1004]">
              <h1 className="mb-[20px] text-2xl font-bold">Delete Item {currentRow}</h1>
              <p className="mb-[20px]">
      ...
      <Modal isOpen={isUpdateOpen} onClose={onUpdateClose} className="!z-[1010]">
        ...
        <ModalContent className="!z-[1002] !m-auto !w-max min-w-[350px] !max-w-[85%] md:top-[12vh]">
          <ModalBody>
            <Card extra="px-[30px] pt-[35px] pb-[40px] max-w-[450px] flex flex-col !z-[1004]">
              <h1 className="mb-[20px] text-2xl font-bold">Update Item</h1>
              <div className="input-wrapper mb-5">
              <InputField
                onChange={handleNameChange}
                value={nameInput}
              ...
              <div className="flex gap-2">
                <button onClick={() => { updateItem()}} className="rounded-xl bg-brand-500 px-5 py-3 text-base font-medium text-white transition duration-200 hover:bg-brand-600 active:bg-brand-700 dark:bg-brand-400 dark:text-white dark:hover:bg-brand-300 dark:active:bg-brand-200">
                  Submit
                </button>
                <button
